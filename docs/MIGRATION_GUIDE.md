...
- **Build backend**: `hatchling` (was: `hatchling` -- already migrated, tooling configs added)
...
- `setup.py` (already removed prior to this migration)
- `setup.cfg` (already removed prior to this migration)
- `pytest.ini` (configuration moved to `[tool.pytest.ini_options]` in `pyproject.toml`)

If these reappear after a sync, delete them again. All configuration is in `pyproject.toml`.

### 3. Source Layout
- **Layout**: `src/` layout
- **Package moved**: `blankly/` -> `src/blankly/`
- **Import unchanged**: `import blankly` still works

If upstream adds files to the old location, move them to `src/blankly/`.
...
3. **Check source layout**: If upstream adds new modules to the old path, move them to `src/blankly/`.
4. **Re-lock**: Run `uv lock` to update `uv.lock` with any new/changed dependencies.
5. **Verify**: Run `uv sync && uv run python -c "import blankly" && uv run pytest` (if tests exist).

## Quick Commands

```bash
uv sync                                    # Install all deps
uv sync --all-extras                       # Install all deps including optional extras
uv run python -c "import blankly"          # Verify import
